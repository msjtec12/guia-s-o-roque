...
export async function generateMetadata({ params }: BusinessPageProps) {
  ...
  const business = await getBusinessBySlug(slug, 'sao-roque');
  if (!business) return constructMetadata({ citySlug: 'sao-roque' });

  return constructMetadata({
    title: `${business.name} em São Roque`,
...
export default async function SaoRoqueBusinessDetailPage({ params }: BusinessPageProps) {
  const { slug } = await params;
  const business = await getBusinessBySlug(slug, 'sao-roque');
  ...
  const allBusinesses = await getBusinesses({ citySlug: 'sao-roque' });
  const relatedBusinesses = allBusinesses
    .filter((b) => b.id !== business.id && b.category_id === business.category_id)
    .slice(0, 3);

  const mapsUrl = buildGoogleMapsUrl(business.address, business.latitude, business.longitude, 'São Roque');
  ...
  const jsonLd = {
    ...
    url: business.website || `https://descubra.tur.br/sao-roque/empresa/${business.slug}`,
    aggregateRating: {
  ...
  const breadcrumbs = [
    ...
    ...(business.category ? [{ label: business.category.name, href: `/sao-roque/explorar?category=${business.category.slug}` }] : []),
    { label: business.name },
  ];
        ...
        <div className="lg:col-span-2 space-y-10">
          ...
          {relatedBusinesses.length > 0 && (
            <section className="space-y-4 pt-6 border-t border-[#E7E5DF]">
              <h2 className="font-serif text-2xl font-bold text-[#26332F]">
                Outros lugares recomendados em São Roque
              </h2>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {relatedBusinesses.map((b) => (
                  <BusinessCard key={b.id} business={b} citySlug="sao-roque" />
                ))}
              </div>
            </section>
          )}

...
export async function generateMetadata({ params }: ExperienceDetailPageProps) {
  ...
  const experience = await getExperienceBySlug(slug, 'sao-roque');
  if (!experience) return constructMetadata({ citySlug: 'sao-roque' });

  return constructMetadata({
    title: `${experience.name} em São Roque`,
...
export default async function SaoRoqueExperienceDetailPage({ params }: ExperienceDetailPageProps) {
  const { slug } = await params;
  const experience = await getExperienceBySlug(slug, 'sao-roque');
  ...
  const breadcrumbs = [
    ...
    { label: 'Experiências', href: '/sao-roque/experiencias' },

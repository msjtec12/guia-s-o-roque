...
export default function SaoRoqueMonteSeuRoteiroPage() {
  ...
  const handleGenerate = async () => {
    ...
    logBusinessEvent({ event_type: 'route_view', page: '/sao-roque/roteiros/montar', city_id: 'city-sao-roque' });
    ...
    const result = await generateCustomItinerary(query, 'sao-roque');
    setItinerary(result);
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20 space-y-8 bg-[#F6F0D4]">
      ...
      <div className="space-y-3 text-center border-b border-[#E7E5DF] pb-6">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white text-[#107492] text-xs font-semibold border border-[#E7E5DF] shadow-xs">
          ...
          <span>Assistente Interativo de Viagem • São Roque</span>
        </div>
        ...
        <p className="text-sm sm:text-base text-[#26332F]/80 max-w-2xl mx-auto">
          Conte o que você mais gosta e descubra uma sugestão personalizada de vinícolas, almoço e passeios no Roteiro do Vinho.
        </p>
      </div>
      ...
      {step === 1 && (
        <div className="bg-white rounded-3xl p-8 border border-[#E7E5DF] shadow-md space-y-8 max-w-2xl mx-auto animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { id: 'casal', label: 'Casal / Romance', icon: Heart, desc: 'A dois no Roteiro do Vinho' },
              { id: 'familia', label: 'Família', icon: Users, desc: 'Com crianças e parquinhos' },
              { id: 'amigos', label: 'Grupo de Amigos', icon: Users, desc: 'Degustação & diversão' },
      ...
      {step === 3 && (
        <div className="bg-white rounded-3xl p-8 border border-[#E7E5DF] shadow-md space-y-8 max-w-2xl mx-auto animate-in fade-in duration-200">
          <div className="space-y-2 text-center">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#26332F]">O que mais combina com você em São Roque?</h2>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              { id: 'vinhos', label: 'Vinhos & Caves', icon: Wine },
              { id: 'gastronomia', label: 'Gastronomia', icon: Utensils },
              { id: 'natureza', label: 'Natureza & Trilhas', icon: Trees },
              { id: 'aventura', label: 'Aventura & Trem', icon: Compass },
              { id: 'compras', label: 'Doces & Empórios', icon: ShoppingBag },
          ...
          <div className="flex justify-between items-center pt-4 border-t border-[#E7E5DF]">
            ...
            >
              ...
              <span>{isGenerating ? 'Gerando Roteiro...' : 'Gerar Roteiro em São Roque'}</span>
            </button>
          </div>
        </div>
      )}

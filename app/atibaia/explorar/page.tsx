...
export const metadata = constructMetadata({
  title: 'Explorar Atibaia | Natureza, Gastronomia e Atrações',
  description: 'Descubra restaurantes, pousadas, voo livre, passeios e pontos turísticos em Atibaia - SP.',
  citySlug: 'atibaia',
});
...
export default async function AtibaiaExplorarPage({ searchParams }: ExplorarPageProps) {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 bg-[#FCFAF5]">
      ...
      <div className="space-y-3 text-center md:text-left border-b border-[#e6dfd4] pb-6">
        <div className="inline-flex items-center gap-2 px-3.5 py-1 rounded-full bg-[#F4EBDD] text-[#183A32] text-xs font-semibold border border-[#e6dfd4]">
          ...
          <span>Guia Completo de Atibaia</span>
        </div>
        <h1 className="font-serif text-3xl sm:text-4xl font-bold text-[#26332F]">
          {activeCategory 
            ? activeCategory.name 
            : tag 
            ? `Atibaia: ${tag.toUpperCase()}`
            : 'Explorar Lugares & Experiências em Atibaia'}
        </h1>
        <p className="text-sm sm:text-base text-[#52615B] max-w-3xl">
          {activeCategory
            ? activeCategory.description
            : 'Encontre os melhores restaurantes, pousadas de montanha, passeios de aventura e atrativos em Atibaia.'}
        </p>
      </div>

      {/* SEARCH BAR */}
      <div className="max-w-3xl mx-auto md:mx-0">
        <SearchBar initialQuery={q} basePath="/atibaia/explorar" />
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8 items-start">
        ...
        {/* SIDEBAR FILTERS */}
        <FilterSidebar categories={categories} totalResults={businesses.length} basePath="/atibaia/explorar" />
          ...
          {businesses.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6">
              {businesses.map((business) => (
                <BusinessCard key={business.id} business={business} citySlug="atibaia" />
              ))}
            </div>
          ) : (
            <div className="bg-white rounded-3xl p-12 text-center border border-[#e6dfd4] shadow-sm space-y-6 max-w-lg mx-auto">
              ...
              <div className="space-y-2">
                <h3 className="font-serif text-xl font-bold text-[#26332F]">
                  Não encontramos lugares com esses filtros em Atibaia
                </h3>
              ...
              <div className="pt-4 border-t border-[#F4EBDD] flex flex-wrap gap-2 justify-center">
                {categories.slice(0, 4).map((cat) => (
                  <Link
                    key={cat.id}
                    href={`/atibaia/explorar?category=${cat.slug}`}

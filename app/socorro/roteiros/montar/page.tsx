...
export default function SocorroMonteSeuRoteiroPage() {
  ...
  const [interests, setInterests] = useState<string[]>(['aventura', 'natureza']);
  ...
  const handleGenerate = async () => {
    ...
    logBusinessEvent({ event_type: 'route_view', page: '/socorro/roteiros/montar', city_id: 'city-socorro' });
    ...
    const result = await generateCustomItinerary(query, 'socorro');
    setItinerary(result);
  ...
  const breadcrumbs = [
    ...
    { label: 'Roteiros', href: '/socorro/roteiros' },
    { label: 'Montar Roteiro' },
  ];

  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 pb-20 space-y-8 bg-[#F6F0D4]">
      
      <Breadcrumbs items={breadcrumbs} />

      {/* HEADER */}
      <div className="space-y-3 text-center border-b border-[#E7E5DF] pb-6">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white text-[#107492] text-xs font-semibold border border-[#E7E5DF] shadow-xs">
          ...
          <span>Assistente Interativo de Viagem • Socorro</span>
        </div>
        <h1 className="font-serif text-3xl sm:text-5xl font-bold text-[#26332F] flex items-center justify-center gap-2">
          ...
          <span>Monte seu roteiro em Socorro</span>
        </h1>
        <p className="text-sm sm:text-base text-[#26332F]/80 max-w-2xl mx-auto">
          Conte suas preferências e descubra uma sugestão personalizada com rafting no Rio do Peixe, Pedra Bela Vista, comida no fogão a lenha e compras de malhas.
        </p>
      </div>
      ...
      {step === 1 && (
        <div className="bg-white rounded-3xl p-8 border border-[#E7E5DF] shadow-md space-y-8 max-w-2xl mx-auto animate-in fade-in duration-200">
          ...
          <div className="grid grid-cols-2 gap-4">
            {[
              { id: 'casal', label: 'Casal / Romance', icon: Heart, desc: 'Pousadas na serra & mirantes' },
              { id: 'familia', label: 'Família', icon: Users, desc: 'Hotéis fazenda & Gruta do Anjo' },
              { id: 'amigos', label: 'Grupo de Amigos', icon: Users, desc: 'Rafting, aventura & chopp' },
              { id: 'sozinho', label: 'Sozinho', icon: User, desc: 'Ecoturismo & trilhas' },
            ].map((option) => {
          ...
          <div className="flex justify-end pt-4 border-t border-[#E7E5DF]">
            <button
              onClick={() => setStep(2)}
              className="inline-flex items-center gap-2 bg-[#F19F14] hover:bg-[#D86E04] text-[#071510] hover:text-[#FFFFFF] font-bold text-sm px-7 py-3.5 rounded-xl shadow-md transition-all cursor-pointer"
            >
      ...
      {step === 2 && (
        <div className="bg-white rounded-3xl p-8 border border-[#E7E5DF] shadow-md space-y-8 max-w-2xl mx-auto animate-in fade-in duration-200">
          <div className="space-y-2 text-center">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#26332F]">Quanto tempo você pretende ficar em Socorro?</h2>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { id: 'horas', label: 'Algumas horas', icon: Clock, desc: 'Rafting ou Pedra Bela Vista' },
              { id: '1dia', label: '1 Dia Completo', icon: Calendar, desc: 'Das 09h às 18h' },
              { id: 'fimdesemana', label: 'Fim de Semana', icon: Hotel, desc: 'Pousada ou Hotel Fazenda' },
            ].map((option) => {
              const Icon = option.icon;
              return (
                <button
                ...
                >
                  <Icon className="w-6 h-6 text-[#107492]" aria-hidden="true" />
          ...
          <div className="flex justify-between items-center pt-4 border-t border-[#E7E5DF]">
            <button
              onClick={() => setStep(1)}
              className="inline-flex items-center gap-1.5 text-xs font-semibold text-[#26332F]/70 hover:text-[#071510] cursor-pointer"
            >
      ...
      {step === 3 && (
        <div className="bg-white rounded-3xl p-8 border border-[#E7E5DF] shadow-md space-y-8 max-w-2xl mx-auto animate-in fade-in duration-200">
          <div className="space-y-2 text-center">
            ...
            <h2 className="font-serif text-2xl font-bold text-[#26332F]">O que mais combina com você em Socorro?</h2>
          </div>

          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {[
              { id: 'rafting', label: 'Rafting no Rio do Peixe', icon: Waves },
              { id: 'mirantes', label: 'Pedra Bela Vista', icon: Mountain },
              { id: 'caipira', label: 'Comida Caipira', icon: Utensils },
              { id: 'malhas', label: 'Feira de Malhas & Tricô', icon: ShoppingBag },
              { id: 'cervejas', label: 'Cervejarias Artesanais', icon: Beer },
              { id: 'gruta', label: 'Gruta do Anjo & Caverna', icon: Compass },
            ].map((option) => {
              ...
              return (
                <button
                  ...
                  className={`p-4 rounded-2xl border text-center font-serif font-bold text-sm transition-all flex items-center justify-between cursor-pointer ${
                    isSelected
                      ? 'border-[#1B4931] bg-[#071510] text-[#FFFFFF] shadow-sm'
                      : 'border-[#E7E5DF] text-[#26332F] hover:bg-[#F6F0D4]/50'
                  }`}
                >
                  <div className="flex items-center gap-2">
                    <Icon className={`w-4 h-4 ${isSelected ? 'text-[#F19F14]' : 'text-[#107492]'}`} aria-hidden="true" />
                    <span>{option.label}</span>
                  </div>
                  {isSelected && <CheckCircle2 className="w-4 h-4 text-[#F19F14] shrink-0" aria-hidden="true" />}
                </button>
              );
            })}
          </div>
          ...
          <div className="flex justify-between items-center pt-4 border-t border-[#E7E5DF]">
            ...
            <button
              ...
              className="inline-flex items-center gap-2 bg-[#F19F14] hover:bg-[#D86E04] text-[#071510] hover:text-[#FFFFFF] font-bold text-sm px-8 py-3.5 rounded-xl shadow-lg transition-all cursor-pointer"
            >
              <Sparkles className="w-4 h-4" aria-hidden="true" />
              <span>{isGenerating ? 'Gerando Roteiro...' : 'Gerar Roteiro em Socorro'}</span>
            </button>
          </div>
        </div>
      )}
        ...
        <div className="space-y-8 animate-in zoom-in-95 duration-300">
          <div className="bg-[#071510] text-[#FFFFFF] rounded-3xl p-8 shadow-xl border border-[#1B4931]/50 flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="space-y-2 text-center md:text-left">
              <span className="text-xs font-bold text-[#F19F14] uppercase tracking-wider block">
                Roteiro Gerado com Sucesso em Socorro
              </span>
          ...
          <div className="max-w-4xl mx-auto space-y-6">
            <h3 className="font-serif text-2xl font-bold text-[#26332F] border-b border-[#E7E5DF] pb-3">
              Cronograma de Paradas Recomendado em Socorro
            </h3>

            <div className="relative border-l-2 border-[#1B4931]/40 pl-6 sm:pl-8 space-y-10">
              {itinerary.stops.map((stop, idx) => {
                const mapsUrl = stop.business
                  ? buildGoogleMapsUrl(stop.business.address, stop.business.latitude, stop.business.longitude, 'Socorro')
                  : '';
                    ...
                    {stop.business && (
                      <div className="pt-2 border-t border-[#E7E5DF] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                        ...
                        <div className="flex flex-wrap items-center gap-2 w-full sm:w-auto">
                          <Link
                            href={`/socorro/empresa/${stop.business.slug}`}

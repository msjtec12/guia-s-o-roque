...
export async function generateMetadata({ params }: BusinessPageProps) {
  ...
  const business = await getBusinessBySlug(slug, 'socorro');
  if (!business) return constructMetadata({ citySlug: 'socorro' });

  return constructMetadata({
    title: `${business.name} em Socorro`,
    description: `${business.description.slice(0, 150)} em Socorro - SP.`,
    image: business.main_image_url,
    citySlug: 'socorro',
  });
}

export default async function SocorroBusinessDetailPage({ params }: BusinessPageProps) {
  const { slug } = await params;
  const business = await getBusinessBySlug(slug, 'socorro');
  ...
  const allBusinesses = await getBusinesses({ citySlug: 'socorro' });
  const relatedBusinesses = allBusinesses
    .filter((b) => b.id !== business.id && b.category_id === business.category_id)
    .slice(0, 3);

  const mapsUrl = buildGoogleMapsUrl(business.address, business.latitude, business.longitude, 'Socorro');
  ...
  const jsonLd = {
    ...
    url: business.website || `https://descubra.tur.br/socorro/empresa/${business.slug}`,
    aggregateRating: {
  ...
  const breadcrumbs = [
    ...
    ...(business.category ? [{ label: business.category.name, href: `/socorro/explorar?category=${business.category.slug}` }] : []),
    { label: business.name },
  ];
            ...
            <section className="space-y-4 pt-6 border-t border-[#E7E5DF]">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                {relatedBusinesses.map((b) => (
                  <BusinessCard key={b.id} business={b} citySlug="socorro" />
                ))}
              </div>
            </section>
          )}

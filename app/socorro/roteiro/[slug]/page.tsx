...
export async function generateMetadata({ params }: RouteDetailPageProps) {
  ...
  const route = await getRouteBySlug(slug, 'socorro');
  if (!route) return constructMetadata({ citySlug: 'socorro' });

  return constructMetadata({
    title: `${route.name} em Socorro`,
...
export default async function SocorroRouteDetailPage({ params }: RouteDetailPageProps) {
  const { slug } = await params;
  const route = await getRouteBySlug(slug, 'socorro');
              ...
              <div key={item.id} className="relative space-y-4">
                ...
                {item.business && (
                  <div className="pt-2 max-w-lg">
                    <BusinessCard business={item.business} citySlug="socorro" />
                  </div>
                )}
              </div>
            ))}
